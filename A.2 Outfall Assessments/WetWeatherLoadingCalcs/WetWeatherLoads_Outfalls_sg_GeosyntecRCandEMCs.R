...
event_total <- dr_rain %>%
  ...
  saveRDS(EMCRainTot2, file = here('A.2 Outfall Assessments/WetWeatherLoadingCalcs/Output/EMCRainTot2.rds')) 
  write_csv(EMCRainTot2, path = here('A.2 Outfall Assessments/WetWeatherLoadingCalcs/Output/EMCRainTot2.csv'))
...
write_csv(SOCWMA_lu, paste0(getwd(), '/A.2 Outfall Assessments/WetWeatherLoadingCalcs/Output/SOCWMA_lu.csv'))
...
stn_RC_202122b <- stn_RC_202122b %>% 
  ...
  mutate(`Stormwater Volume (acre-ft)_RC_Actual` = RC_Actual * area_acres * (Rain_Gage_Total/ 12)
  ) %>%
...
stn_load <- stn_load2 %>%    
 ...
 filter(!is.na(`Stormflow Volume -cf`)) %>% 
  ...
  mutate(
    Load_Annual = `Stormwater Volume (acre-ft)_RC_M`*Result
  ) %>% 
...
saveRDS(EMC_M, file = here('A.2 Outfall Assessments/WetWeatherLoadingCalcs/Output/EMC_M.rds'))
write_csv(EMC_M, path = here('A.2 Outfall Assessments/WetWeatherLoadingCalcs/Output/EMC_M.csv')) 
...
saveRDS(EMC_CF, file = here('A.2 Outfall Assessments/WetWeatherLoadingCalcs/Output/EMC_CF.rds'))
write_csv(EMC_CF, path = here('A.2 Outfall Assessments/WetWeatherLoadingCalcs/Output/EMC_CF.csv')) 
...
saveRDS(EMC_WMAb, file = here('A.2 Outfall Assessments/WetWeatherLoadingCalcs/Output/EMC_WMAb.rds'))
write_csv(EMC_WMAb, path = here('A.2 Outfall Assessments/WetWeatherLoadingCalcs/Output/EMC_WMAb.csv')) 
...
saveRDS(EMC_WMA, file = here('A.2 Outfall Assessments/WetWeatherLoadingCalcs/Output/EMC_WMA.rds'))
write_csv(EMC_WMA, path = here('A.2 Outfall Assessments/WetWeatherLoadingCalcs/Output/EMC_WMA.csv')) 
...
# tribformat2<-tribformat2[, c("FACILITYID", "Area_Rainfall", "Trib_LU_Volume", "EMC_Group", "RC_Group", "EMC_Group_Area", "RC", "RC_M", "Copper, Total - Dissolved - mg/L" :"Zinc, Total - Total - mg/L:")]
 ...
 write_csv(tribformat2, paste0(getwd(), '/A.2 Outfall Assessments/WetWeatherLoadingCalcs/Output/TribLoadsLU_Geosyntec.csv'))
 ...
 tribformat3<-  #get RC_WMA (one RC for the entire WMA)
  ...
  write_csv(tribformat3, paste0(getwd(), '/A.2 Outfall Assessments/WetWeatherLoadingCalcs/Output/TribLoadsLU_Geosyntec.csv'))
